...
def register():
    ...
    def sub():
        cv2.imwrite("known_faces/" + entry.get() +
                    ".jpg", cv2.imread(img.get()))
        reg.destroy()
    ...
    label = customtkinter.CTkLabel(
        reg, text="Roll Number :", font=("Courier", 16))
    label.pack()
    ...
    #     reg, text="Upload Image", font=("Courier", 16), command=file)
    # img_button.pack()
    # cam = customtkinter.CTkLabel(reg)
    # cam.place(x=10, y=0, width=700, height=50)
    # cam.pack()
    submit = customtkinter.CTkButton(
        reg, text="Submit", font=("Courier", 16), command=sub)
    submit.pack(pady=20)
    reg.mainloop()
...
def mark():
    ...
    def sub():
        cv2.imwrite("unknown_faces/" + entry.get() +
                    ".jpg", cv2.imread(img.get()))
        ...
        if main.compare("known_faces/" + entry.get() + ".jpg", "unknown_faces/" + entry.get() + ".jpg") == True:
            out = customtkinter.CTkLabel(
                final, text=entry.get() + " Logged In", font=("Courier", 16))
            out.pack(pady=15)
        else:
            out = customtkinter.CTkLabel(
                final, text="Unknown User/Register First", font=("Courier", 16))
            out.pack(pady=15)
    ...
    label = customtkinter.CTkLabel(
        mar, text="Roll Number :", font=("Courier", 16))
    label.pack()
    ...
    img.pack(pady=20)
    img_button = customtkinter.CTkButton(
        mar, text="Upload Image", font=("Courier", 16), command=file)
    img_button.pack()
    submit = customtkinter.CTkButton(
        mar, text="Submit", font=("Courier", 16), command=sub)
    submit.pack(pady=20)
    mar.mainloop()
...
register_button = customtkinter.CTkButton(
    root, text="Register", font=("Courier", 16), command=register)
register_button.pack(pady=10)

mark_attendance_button = customtkinter.CTkButton(
    root, text="Mark Attendance", font=("Courier", 16), command=mark)
